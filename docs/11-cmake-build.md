...
-> 边编译边修
-> 产出 .so
-> install 产物中的上游自带、可独立运行的测试入口
-> build 目录里的上游自带、可独立运行的测试入口
-> 上游 CLI 能力校验
-> 若无现成测试入口则记录“无测试用例”
-> mcp 设备测试
-> 失败则 hdc fallback
...
1. install 产物中的上游自带、可独立运行的测试入口
2. 构建目录里的上游自带、可独立运行的测试入口
3. 上游 CLI

说明：
- 如果 install 产物中没有 binary，但构建目录里已经生成上游自带、可独立运行的测试入口，不应直接判定 `binary-pass` 失败
- 应优先从构建目录回收该 binary 到 `outputs/<库名>/bin/`
- 如果上游自带的测试入口不适合设备侧运行，但上游存在 CLI，则应优先使用 CLI 做真实能力校验，不能只跑 `--version` / `-V`
- CLI 能力校验必须至少覆盖一条真实功能路径，例如压缩/解压回环、解析/转换、输入输出处理等
- 如果上游没有现成测试入口或 CLI，则直接在报告中记录“无测试用例”
